    ...
    <header className="sticky top-0 z-30 border-b border-[var(--color-border)] bg-[var(--color-background)]/85 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-4">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="font-display text-2xl leading-none">Stonebrae Match</span>
        </Link>
        <nav className="flex items-center gap-1 sm:gap-2">
          <Link
            href="/leaderboard"
            className="rounded-lg px-3 py-2 text-sm font-medium text-[var(--color-muted)] hover:text-[var(--color-foreground)]"
          >
            Leaderboard
          </Link>
          {player && (
